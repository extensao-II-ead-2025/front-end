...
BEGIN
  ...
  INSERT INTO public.categories (user_id, name, type, icon, color) VALUES
    (p_user_id, 'Alimentação', 'expense', 'utensils', '#EF4444'),
    (p_user_id, 'Transporte', 'expense', 'car', '#F59E0B'),
    (p_user_id, 'Moradia', 'expense', 'home', '#8B5CF6'),
    (p_user_id, 'Saúde', 'expense', 'heart-pulse', '#EC4899'),
    (p_user_id, 'Educação', 'expense', 'graduation-cap', '#3B82F6'),
    (p_user_id, 'Lazer', 'expense', 'smile', '#10B981'),
    (p_user_id, 'Vestuário', 'expense', 'shirt', '#6366F1'),
    (p_user_id, 'Contas', 'expense', 'file-text', '#EF4444'),
    (p_user_id, 'Outros', 'expense', 'more-horizontal', '#6B7280')
  ON CONFLICT (user_id, name, type) DO NOTHING;

  -- Categorias de RECEITAS (income)
  INSERT INTO public.categories (user_id, name, type, icon, color) VALUES
    (p_user_id, 'Salário', 'income', 'briefcase', '#10B981'),
    (p_user_id, 'Freelance', 'income', 'code', '#3B82F6'),
    (p_user_id, 'Investimentos', 'income', 'trending-up', '#8B5CF6'),
    (p_user_id, 'Presente', 'income', 'gift', '#EC4899'),
    (p_user_id, 'Outros', 'income', 'dollar-sign', '#10B981')
  ON CONFLICT (user_id, name, type) DO NOTHING;
END;
